...
// First difference wins: lamports, then owner, then executable, then data.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MismatchKind {
    ...
    // Data differs; carries both lengths and the first differing offset.
    Data {
        engine_len: usize,
        snapshot_len: usize,
...
// Compares lamports/owner/executable/data (the lattice fields). rent_epoch excluded: lattice ignores it, rent disabled at the epoch-808 floor. Zero-lamport = absent on both sides.
...
mod tests {
    ...
    // Slots differ on purpose, the diff compares values, not write slots.
